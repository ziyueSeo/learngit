...
int main()
{
	...
	/*
	lhc comments: 文件打不开时，应该停止下面的操作，直接return -1
	*/
	...
	stri.push_back("\0");
	//避免出现像34这种东西，有很强的人为设计性。好的代码应该是针对不同数据都能用的
	...
	//这个用map最简单直接。
	//为什么？因为map里面的存储的key都是排序好的。
	//如果你用线性数组，对于每个单词，查找的频率是O(n)
	//但是如果你用map, 每个单词的查找频率是O(log2n)
